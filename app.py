...
def preprocess_imagewheat(image, target_size=(64, 64)):
    try:
...
def preprocess_imagemaize(image, target_size=(244,244)):
    try:
        img = tf.keras.utils.img_to_array(image) / 255.0  # Normalize to [0, 1]
        img = tf.image.resize(img, target_size)
        return np.expand_dims(img, axis=0)  # Add batch dimension
...
def predict():
    ...
    if image_file:
        ...
        if crop_type == 'wheat':
            img_array = preprocess_imagewheat(image)
            model = wheat_model
            class_names = wheat_class_names
        elif crop_type == 'maize':
            img_array = preprocess_imagemaize(image)
            model = maize_model
